...
\begin{definition}[Hausdorffova metrika]\label{def:hausdorffova-metrika}
    Hausdorffovou metrikou\index{Hausdorffův metrický prostor!Hausdorffova metrika} (vzdálenost) nazýváme takové zobrazení $\mapping{\hausdorffmetric}{\powset{X}\times\powset{X}}{\langle0,\infty\rangle}$,~kde pro každé $A,B\in\powset{X}$ platí
    \[\hausdorffmetric(A,B)=\inf\set{\delta>0\;\middle|\;A\subseteq B_\delta\;,\;B\subseteq A_\delta}.\]
\end{definition}
...
Pokud se však omezíme jen na některé množiny,~bude $\hausdorffmetric$ skutečně metrikou ve smyslu definice \todo{doplnit odkaz}. Proto se dále zaměříme pouze na takové podmnožiny $X$,~které jsou \emph{neprázdné} a \emph{kompaktní}.
\begin{definition}[Hyperprostor]\label{def:hyperprostor}
    Systém všech neprázdných kompaktních podmnožin množiny $X$ nazýváme \emph{hyperprostor}\index{hyperprostor} a značíme jej $\mathbb{H}(X)$.
\end{definition}
\todo{Je vhodné značení $\mathbb{H}(X)$,~nebo se v~české literatuře používá jiné?}
\begin{proposition}\label{prop:sjednoceni-kompaktnich-mnozin}
    Nechť $A,B\in\mathbb{H}(X)$. Pak $A\cup B\in\mathbb{H}(X)$.
\end{proposition}
\begin{proof}
...
\begin{corollary}\label{cor:sjednoceni-n-kompaktnich-mnozin}
    Pro $A_1,A_2,\ldots,A_n\in\mathbb{H}(X)$ platí $\bigcup_{i=1}^n A_i\in\mathbb{H}(X)$.
\end{corollary}
...
\begin{theorem}\label{thm:hausdorffova-metrika-je-metrika}
    Nechť $(X,\varrho)$ je metrický prostor, kde $X\subseteq\R^n$. Pak Hausdorffova metrika $\hausdorffmetric$ je metrikou na $\mathbb{H}(X)$.
\end{theorem}
\begin{proof}
    Zjevně pro každé $A,B\in\mathbb{H}(X)$ z~definice platí $\hausdorffmetric(A,B)\geqslant 0$ a $\hausdorffmetric(A,B)=\hausdorffmetric(B,A)$. Z~kompaktnosti množin $A,B$ plyne (podle věty \todo{doplnit odkaz}),~že $A,B$ jsou omezené množiny,~tedy $\hausdorffmetric(A,B)<\infty$.
    
    Pro $A=B$ platí,~že pro každé $\delta>0$ je splňeno $A\subseteq B_\delta$ a zároveň $B\subseteq A_\delta$,~tzn. $\hausdorffmetric(A,B)=0$. Opačná implikace také platí. Předpokládejme,~že množiny $A,B$ splňují $\hausdorffmetric(A,B)=0$. Pokud $x\in A$,~pak pro každé $\delta>0$ platí,~že $x\in B_\delta$,~tzn. $\varrho(x,B)=0$,~a z~uzavřenosti $B$ plyne $x\in B$. Tedy $A\subseteq B$ a analogicky platí i~$B\subseteq A$,~tzn. $A=B$.
...
\todo{Je potřeba předpokládat, že $X\subseteq\R^n$? }
\begin{definition}[Hausdorffův metrický prostor]\label{def:hausdorffuv-mp}
    Metrický prostor $(\mathbb{H}(X),\hausdorffmetric)$~nazýváme \emph{Hausdorffův metrický prostor}\index{Hausdorffův metrický prostor} na metrickém prostoru $(X,\varrho)$.
\end{definition}
