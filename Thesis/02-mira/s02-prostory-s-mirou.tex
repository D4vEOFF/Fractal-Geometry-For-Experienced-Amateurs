...
\begin{definition}[$\sigma$-algebra]\label{def:sigma-algebra}
    Nechť $X$ je libovolná množina a systém podmnožin $\mathcal{A}\subseteq\powset{X}$. Pak $\mathcal{A}$ je \emph{$\sigma$-algebra}\index{teorie míry!$\sigma$-algebra} na množině $X$, pokud:
...
\begin{definition}[Prostor s~mírou]\label{def:prostor-s-mirou}
    Nechť $\mathcal{A}$ je $\sigma$-algebra na množině $X$. Zobrazení $\mapping{\mu}{\mathcal{A}}{\langle0,\infty\rangle}$ se nazývá \emph{míra}\index{teorie míry!míra} na $\mathcal{A}$, pokud platí:
    \begin{enumerate}[label=(\roman*)]
    ...
    \end{enumerate}
    Uspořádanou trojici $(X,\mathcal{A},\mu)$ nazýváme \emph{prostor s~mírou}\index{teorie míry!prostor s mírou}.
\end{definition}
