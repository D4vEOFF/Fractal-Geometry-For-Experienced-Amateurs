...
\begin{definition}[$\sigma$-algebra]\label{def:sigma-algebra}
    Nechť $X$ je libovolná množina a systém podmnožin $\mathcal{A}\subseteq\powset{X}$. Pak $\mathcal{A}$ je \emph{$\sigma$-algebra}\index{měřitelný prostor!$\sigma$-algebra} na množině $X$, pokud:
...
\begin{theorem}[Vlastnosti míry]\label{thm:mira-vlastnosti}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item\label{thm:mira-nekl-posl} Pokud $A_1,A_2,\ldots$, kde $A_i\in\mathcal{A}$ pro každé $i\in\N$, je neklesající posloupnost množin, tj. taková, že $A_j\subseteq A_{j+1}$ pro každé $j\in\N$, pak
