...
Pro formální zavedení tohoto pojmu však budeme muset nejprve zavést ještě jiný pojem,~a to \emph{$\sigma$-algebru}.
...
\begin{example}\label{ex:mira}
    ...
    \begin{itemize}
        ...
        \[\delta_x(A)=\chi_A(x)=\begin{cases}
            ...
            0 & x\notin A.
        \end{cases}\]
        Zobrazení $\delta_x$ je tzv. \emph{Diracova míra}\index{míra!Diracova}\index{Diracova míra}.
        \item Zobrazení přiřazující náhodnému jevu\index{náhodný jev}\index{jev!náhodný} pravděpodobnost\index{pravděpodobnost} je též případem míry. Označíme-li si $\Omega=\set{\omega_1,\omega_2,\ldots,\omega_n}$ množinu všech elementárních jevů a~$\sigma$-algebru $\mathcal{F}\subseteq\powset{\Omega}$,~pak $\mapping{\mathsf{P}}{\mathcal{F}}{\langle0,1\rangle}$ definovaná pro $A\in\mathcal{F}$ jako
