...
\begin{lemma}\label{lem:hausdorffova-mira-sigma-aditivita-kladna-vzdalenost}
    Nechť $(X,\varrho)$ je metrický prostor a $A,B\subseteq X$, takové, že pro jejich vzdálenost platí $\varrho(A,B)>0$. Pak pro každé $s\geqslant 0$ platí
...
\begin{proof}
    ...
    \[\sum_{i=1}^{\infty}(\diam{F_i})\leqslant\hausdorffmeasure{s}(A\cup B)+\varepsilon.\]
...
\begin{lemma}\label{lem:hausdorffova-mira-sigma-aditivita-temer-disjunktni}
    Nechť $(X,\varrho)$ je metrický prostor a $A,B\subseteq X$ jsou téměř disjunktní množiny o průměru nejvýše $\delta>0$. Pak pro každé $s\geqslant 0$ platí
    \[\hausdorffmeasure{s}(A\cup B)=\hausdorffmeasure{s}(A)+\hausdorffmeasure{s}(B).\]
\end{lemma}
